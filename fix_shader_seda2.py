...
# Reemplazar el bloque SHADER FONDO completo usando regex
m = re.search(r'// SHADER FONDO\s*// =+\s*\(function\(\)\{.*?\}\)\(\);', c, re.DOTALL)
if not m:
    # intentar sin la linea de iguales
    m = re.search(r'// SHADER FONDO.*?\(function\(\)\{.*?\}\)\(\);', c, re.DOTALL)

if m:
    nuevo = """// SHADER SEDA — Cantabrana v22
// ================================================================
(function(){
...
void main(){
  ...
  // Color dinamico texto loading
  const claras=[
    {c:'#ffffff',b:'rgba(255,255,255,0.08)'},
    {c:'#f0c060',b:'rgba(240,192,96,0.10)'},
    {c:'#d0d8e8',b:'rgba(208,216,232,0.08)'},
  ];
  function aplColor(){
    const ci=Math.floor(Date.now()/4000)%claras.length;
    const col=claras[ci];
    const sh='0 2px 10px rgba(0,0,0,0.95)';
    ['ld-btn'].forEach(id=>{const el=document.getElementById(id);if(el){el.style.color=col.c;el.style.borderColor=col.c;el.style.background=col.b;}});
    document.querySelectorAll('.ld-titulo,.ld-subtitulo,.ld-autor').forEach(el=>{el.style.color=col.c;el.style.textShadow=sh;});
  }
  setInterval(aplColor,400);aplColor();
})();"""
    c = c[:m.start()] + nuevo + c[m.end():]
    print('OK shader seda aplicado')
else:
    print('ERR: bloque no encontrado')
